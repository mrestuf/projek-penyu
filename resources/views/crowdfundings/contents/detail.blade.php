    ...
    <div class="p-4">
        <ol
            ...
            <li class="flex items-center">
                ...
                <a href="/pengembangan">Pengembangan Wisata</a>
                <svg aria-hidden="true" class="w-4 h-4 ml-2 sm:ml-4" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 5l7 7-7 7M5 5l7 7-7 7">
                    </path>
            ...
            <li class="flex items-center text-blue-600 dark:text-blue-500">
                ...
                {{ $crowdfunding->nama_pengembangan_wisata }}
            </li>
        </ol>

        {{-- <div class="flex flex-wrap md:-m-2 -m-1">
            <div class="grid grid-cols-2 w-full">
                @for ($i = 0; $i < count($crowdfunding->images); $i++)
                    @if ($i == 0)
                    <div class="md:p-2 p-1">
                        <img alt="gallery" class="object-cover w-full h-full object-center block"
                            src="{{ asset('storage/' . $crowdfunding->images[$i]) }}">
                    </div>
                    @endif
                    @if($i == 1)
                    <div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-6 gap-5">
            <div class="py-3 lg:py-10 lg:col-span-4">
                <div class="flex justify-between">
                    <h1 class="text-1xl md:text-4xl font-semibold">{{ $crowdfunding->nama_pengembangan_wisata }}</h1>
                    {{-- <h1 class="text-1xl md:text-3xl font-semibold">
                        {{ \App\Helpers\PriceFormat::price($crowdfunding->price) }}/Tiket</h1> --}}
                </div>
                <p class="mt-10 text-sm">{!! $crowdfunding->status !!}</p>
            </div>
                    ...
                    <div class="mx-auto">
                        <div class="bg-white shadow-lg rounded-lg mt-9">
                            ...
                            <header class="text-center px-5 pb-5">
                                <!-- Card name -->
                                <h3 class="text-xl font-bold text-gray-900 mb-1">Crowdfunding</h3>
                                <p class="text-sm text-gray-500">Fill in your details and review your transaction.</p>
                            </header>
                            <!-- Card body -->
                            <div class="bg-gray-100 px-5 py-6">
                                <h1 class="my-3 text-left text-lg font-semibold">Detail Transaction</h1>
                                <div class="flex justify-between">
                                    <h6 class="m-0">Target Pengembangan</h6>
                                    <p class="m-0">{{ \App\Helpers\PriceFormat::price($crowdfunding->target_pengembangan)}}</p>
                                </div>
                                <div class="flex justify-between">
                                    <h6 class="m-0">Bagi Hasil</h6>
                                    <p class="m-0">0%</p>
                                </div>
                                <div class="flex justify-between">
                                    <h6 class="m-0">Minimal Invest</h6>
                                    <p class="m-0">{{\App\Helpers\PriceFormat::price(0)}}</p>
                                </div>
                                <div class="flex justify-between">
                                    <h6 class="m-0">Terkumpul</h6>
                                    <p class="m-0">{{ \App\Helpers\PriceFormat::price($crowdfunding->total_pengembangan)}}</p>
                                </div>
                                <div class="w-full bg-gray-200 rounded h-6 dark:bg-gray-700">
                                    <div class="bg-blue-600 h-6 rounded" style="width: {{(($crowdfunding->target_pengembangan - ($crowdfunding->target_pengembangan-$crowdfunding->total_pengembangan))/$crowdfunding->target_pengembangan)*100}}%">{{(($crowdfunding->target_pengembangan - ($crowdfunding->target_pengembangan-$crowdfunding->total_pengembangan))/$crowdfunding->target_pengembangan)*100}}%</div>
                                  </div>
                                @if (Auth::check())
                                <form action="/pengembangan/{{ $crowdfunding->id }}/invoice" method="POST" class="space-y-3">
                                    @csrf
